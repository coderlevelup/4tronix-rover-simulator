...
export function getFirestoreClient(): Firestore {
  if (!firestore) {
    initializeFirebaseClient();
    // Force long polling (plain HTTPS) — the default gRPC/WebChannel streaming
    // transport is blocked by many networks (VPNs, ad-blockers, firewalls) and
    // by Node, causing "Could not reach Cloud Firestore backend ... offline
    // mode". Long polling gets through.
    try {
