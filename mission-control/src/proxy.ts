...
/**
 * Verify a Firebase session token (ID token) against Google's published
 * x509 certificates, with caching. Edge-compatible (no node:crypto / Buffer).
 */
...
export async function proxy(request: NextRequest) {
  ...
  // Page routes redirect to /login; API routes get a JSON 401 so fetch clients
  // see an auth error instead of an HTML redirect.
